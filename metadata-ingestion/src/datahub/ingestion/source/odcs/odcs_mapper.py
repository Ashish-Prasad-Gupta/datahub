...
from typing import Dict, Iterable, List, Optional, Tuple, TypeGuard
...
def _custom_props_for_rule(ctx: _RuleContext) -> Dict[str, str]:
    ...
    for key, value in (
        ...
        if value:
            props[key] = value
    # arguments stays special-cased: serialized JSON with a graceful fallback.
    if rule.arguments:
        try:
            props[PROP_RULE_ARGUMENTS] = json.dumps(rule.arguments, sort_keys=True)
        except (TypeError, ValueError):
            pass
    return props
